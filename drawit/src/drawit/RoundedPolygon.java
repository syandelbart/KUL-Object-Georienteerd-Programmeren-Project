...
public class RoundedPolygon {
	 ...
	 * 	| IntStream.range(index+1, getVertices().length).allMatch(i -> old(getVertices()[i-1]).equals(getVertices()[i])
	 */
	public void insert(int index,IntPoint point) {
		if(!(index <= this.vertices.length) || PointArrays.checkDefinesProperPolygon(PointArrays.insert(this.vertices,index,point)) != null) {
			throw new IllegalArgumentException();
		} else {
	...
	public void remove(int index) {
		if(!(index <= this.vertices.length-1) || !(this.vertices.length > 3) || PointArrays.checkDefinesProperPolygon(PointArrays.remove(this.vertices,index)) != null) {
			throw new IllegalArgumentException();
		} else {
	...
	public void update(int index,IntPoint point) {
		if(!(index <= this.vertices.length-1) || PointArrays.checkDefinesProperPolygon(PointArrays.update(this.vertices,index,point)) != null) {
			throw new IllegalArgumentException();
		} else {
